...
// p = 0.0 hard left, 0.5 center, 1.0 hard right.
// Constant-power law: gL^2 + gR^2 == 1 for every p, so the source
// does not thin out as it crosses center.
Stereo pan(double sample, double p) {
    if (p < 0.0) p = 0.0;
    if (p > 1.0) p = 1.0;

    double angle = p * M_PI / 2;
...
void render_file(string filename, string type, double start_frequency, 
    ...
    if (pan_rate < 0) {
        cerr << "pan_rate must be >= 0, got " << pan_rate << "\n";
        return;
    }
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (pan_rate > 0) {
            pan_phase += pan_rate / sample_rate;
            pan_phase -= floor(pan_phase);

            // sin() is continuous across the pan_phase 1 -> 0 wrap,
            // so there is no click at the loop point.
            double p = 0.5 * (1.0 + sin(2 * M_PI * pan_phase));

            Stereo out = pan(sample, p);
            channel1 = out.left;
            channel2 = out.right;
        }

        write_as_bytes(wav, (int) lround(channel1), 2);
        write_as_bytes(wav, (int) lround(channel2), 2);
    }
...
int main() {
    ...
    // pan_rate is the last argument: 0.5 Hz = one revolution every 2 seconds.
    render_file("sine_pan.wav", "sine", 220, 8, false, 0, 0.5);
    render_file("sawtooth_pan.wav", "sawtooth", 220, 8, false, 0, 0.5);
